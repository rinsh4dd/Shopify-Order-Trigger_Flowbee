...
export default function Index() {
  ...
  const activeTemplateOrderCreated =
    ...
    <div className="font-sans bg-flowbee-50 min-h-screen px-10 py-8 text-gray-900 box-border max-md:px-5">
      ...
      <div className="flex justify-between items-center mb-8 border-b border-flowbee-200 pb-5 max-sm:flex-col max-sm:items-start max-sm:gap-4 max-sm:pb-4">
        <div className="flex items-center gap-4 max-sm:gap-3">
          ...
          />
          <div>
            <h1 className="font-display text-[22px] font-extrabold text-flowbee-900 m-0 max-sm:text-lg">
              Flowbee WhatsApp Notifications
            </h1>
            <p className="text-[13px] text-flowbee-700 mt-1 m-0">
              Monitor connection status, order event automations, and recovery
              templates.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 max-sm:w-full max-sm:justify-start max-sm:flex-wrap max-sm:gap-2.5">
          <span className="bg-flowbee-100 text-flowbee-900 text-[13px] font-semibold px-3 py-1.5 rounded-full border border-flowbee-200">
            {shop}
          </span>
          <span
            className={`inline-flex items-center gap-2 text-[13px] font-semibold px-3.5 py-1.5 rounded-full ${isConnected ? "bg-emerald-100 text-emerald-800 border border-emerald-200" : "bg-red-100 text-red-800 border border-red-300"}`}
          >
            <span
              className={`w-2 h-2 rounded-full inline-block animate-pulse-dot ${isConnected ? "bg-emerald-500" : "bg-red-500"}`}
            ></span>
      ...
      <div className="bg-white border border-flowbee-200 rounded-2xl px-6 py-4 mb-6 flex items-center justify-between shadow-[0_4px_12px_rgba(245,158,11,0.02)] w-full box-border max-sm:flex-col max-sm:items-start max-sm:gap-4 max-sm:p-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <strong className="text-flowbee-900 text-[14.5px] font-bold block">
              Configurations Overview
            </strong>
            <p className="text-flowbee-700 text-[13px] mt-1 m-0">
              These are the settings currently live in your Shopify store
              database. Click Edit below to modify API credentials, numbers, or
              templates.
            </p>
          </div>
        </div>
        <Link
          to="/app/settings"
          className="bg-gradient-to-br from-flowbee-500 to-flowbee-600 text-white! no-underline px-5.5 py-2.5 rounded-xl text-[13.5px] font-bold transition-all whitespace-nowrap inline-flex items-center gap-2 shadow-[0_4px_10px_rgba(245,158,11,0.15)] hover:-translate-y-px hover:shadow-[0_6px_14px_rgba(245,158,11,0.22)] max-sm:w-full max-sm:justify-center max-sm:box-border"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="16"
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-5 mb-8">
        <div className="bg-white border border-flowbee-200 rounded-2xl p-6 flex items-center gap-5 shadow-[0_4px_6px_rgba(0,0,0,0.01)] transition-all hover:shadow-[0_10px_15px_-3px_rgba(245,158,11,0.05)] hover:border-flowbee-500 hover:-translate-y-0.5">
          ...
          <div className="flex flex-col">
            <span className="text-[13px] text-flowbee-700 font-medium">
              WhatsApp Sender
            </span>
            <span className="text-base font-bold text-flowbee-900 mt-1">
              {formatPhone(settings?.flowbeeRegisteredPhone)}
            </span>
          </div>
        </div>
        ...
        <div className="bg-white border border-flowbee-200 rounded-2xl p-6 flex items-center gap-5 shadow-[0_4px_6px_rgba(0,0,0,0.01)] transition-all hover:shadow-[0_10px_15px_-3px_rgba(245,158,11,0.05)] hover:border-flowbee-500 hover:-translate-y-0.5">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-base font-bold text-flowbee-900 mt-1">
              {formatPhone(settings?.flowbeeNotifyPhone)}
            </span>
          </div>
        </div>
        ...
        <div className="bg-white border border-flowbee-200 rounded-2xl p-6 flex items-center gap-5 shadow-[0_4px_6px_rgba(0,0,0,0.01)] transition-all hover:shadow-[0_10px_15px_-3px_rgba(245,158,11,0.05)] hover:border-flowbee-500 hover:-translate-y-0.5">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-base font-bold text-flowbee-900 mt-1">
              {formatDelay(settings?.flowbeeAbandonedCartDelay)}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-[2fr_1fr] gap-8 max-lg:grid-cols-1">
            ...
            <div className="p-7">
              <div className="flex justify-between border-b border-flowbee-100 py-3.5">
                <span className="text-[13.5px] text-gray-600 font-medium">
                  Recovery Template ID
                </span>
                <span className="text-[13.5px] font-semibold text-gray-800">
                  {settings?.flowbeeTemplateAbandonedCart || "Not configured"}
                </span>
              </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white border border-flowbee-200 rounded-[20px] shadow-[0_4px_6px_rgba(0,0,0,0.01)] overflow-hidden">
            <div className="px-7 py-6 border-b border-flowbee-100 flex justify-between items-center">
              <h2 className="font-display text-base font-bold text-flowbee-900 flex items-center gap-2.5 m-0">
                Setup & Help Checklist
              </h2>
            </div>
            <div className="p-6">
              <WidgetItem
                title="Get Flowbee API Key"
                description="Obtain your secret API key from the Flowbee API platform under Settings."
              />
              <WidgetItem
                title="Register WhatsApp Number"
                description="Specify the sender phone number associated with your WhatsApp business profile."
              />
              <WidgetItem
                title="Select Approved Templates"
                description="Choose WhatsApp-approved notification templates for order logs and recovering checkouts."
                isLast
              />
            </div>
          </div>
              ...
              <div className="flex flex-col gap-3">
                {activityLogs.length === 0 ? (
                  <div className="text-center text-gray-500 text-[13px] py-2.5">
                    No activity events yet. Events will appear here once orders
                    are placed or abandoned checkouts are detected.
                  </div>
                ) : (
                  activityLogs.map((log) => {
                    ...
                    const cleanDetail = (log.detail || "").replace(
                      " for Customer",
                      "",
                    );
                    ...
                    return (
                      <div
                        className="p-3 rounded-[10px] bg-gray-50 border border-gray-100 flex items-center gap-3 text-xs"
                        key={log.id}
                      >
                        <span
                          className={`font-bold text-[10px] px-1.5 py-0.5 rounded uppercase ${badgeClass}`}
                        >
                          {badgeText}
                        </span>
                        <div className="grow">
      ...
      <footer className="mt-12 border-t border-flowbee-200 pt-5 flex justify-between items-center text-[13px] text-flowbee-700 max-sm:flex-col max-sm:gap-3 max-sm:items-center max-sm:text-center">
        <p>
          &copy; {new Date().getFullYear()} Flowbee.io. All rights reserved.
        </p>
        <div className="flex items-center gap-2.5">
          <a
            href="https://flowbee.io"
            target="_blank"
            rel="noopener noreferrer"
            className="text-flowbee-600 no-underline font-medium transition-colors hover:text-flowbee-700"
          >
            Website
          </a>
          <span>&bull;</span>
          <a
            href="mailto:support@flowbee.io"
            className="text-flowbee-600 no-underline font-medium transition-colors hover:text-flowbee-700"
          >
            Support
          </a>
...
function AutomationRow({ label, templateId, icon, isLast = false }) {
  ...
  return (
    <div
      className={`flex justify-between items-center px-5 py-4 rounded-xl border border-flowbee-100 bg-gray-50 transition-all hover:border-flowbee-500 hover:bg-flowbee-50 ${isLast ? "" : "mb-3.5"}`}
    >
      <div className="flex items-center gap-3.5">
        <div
          className={`w-10 h-10 rounded-[10px] flex items-center justify-center ${enabled ? "bg-emerald-50 text-emerald-600" : "bg-gray-100 text-gray-500"}`}
        >
          {icon}
        </div>
        <div>
      ...
      <span
        className={`text-xs font-semibold px-2.5 py-1 rounded-full ${enabled ? "bg-emerald-100 text-emerald-800" : "bg-gray-100 text-gray-600"}`}
      >
...
function WidgetItem({ title, description, isLast = false }) {
  return (
    <div className={`flex gap-3 ${isLast ? "" : "mb-4"}`}>
      ...
      <div>
        <h4 className="text-[13.5px] font-semibold text-gray-800 m-0">
          {title}
        </h4>
        <p className="text-[12.5px] text-gray-500 mt-1 m-0 leading-relaxed">
          {description}
        </p>
      </div>
    </div>
